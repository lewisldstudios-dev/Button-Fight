...
function checkIfEnd() {
    if (currentLevel >= 10) {
        window.alert('You killed every enemy! I hope you enjoyed the game!')
        localStorage.removeItem('currentLevel')
        localStorage.removeItem('playerStrenght')
        localStorage.removeItem('coins')
        localStorage.removeItem('heal')
    }
}

function checkIfPlayed() {
    if (coins > 1 || currentLevel > 1) {
        window.location.href = 'shop.html'
    } else {
        window.alert('You have to play the first level before visiting the shop!')
    }
}
